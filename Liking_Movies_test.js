...
Scenario('searching movies', async ({ I }) => {
  ...
  const searchQuery = titles[1].substring(1, 2);
  ...
  for (let i = 0; i < matchingMovies.length; i++) {
    ...
    if (matchingMovies.includes(visibleTitle)) {
      sortMovies.push(visibleTitle);
    }

    assert.strictEqual(sortMovies[i], visibleTitle);
  }
});
